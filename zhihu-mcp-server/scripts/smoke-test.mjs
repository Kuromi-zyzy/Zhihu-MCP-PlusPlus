...
try {
  ...
  add('auth status', a.ok === true, `logged_in=${a.data?.logged_in}`);
  ...
  add('bing search', b.ok === true, `total=${b.data?.total}`);
  ...
  add('diagnostics', d.ok === true && d.data?.sqlite?.ok, `bing=${d.data?.bing?.reachable}`);
